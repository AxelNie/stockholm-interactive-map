...
const PopupInfo = ({ onClose, isMobileDevice }: { onClose: () => void, isMobileDevice: boolean }) => {
    // Generate the gradient
    const [page, setPage] = useState(0);
    const [lastPageOnMobile, setLastPageOnMobile] = useState<boolean>(isMobileDevice && page == pages.length - 1);
    ...
    useEffect(() => {
        ...
        console.log("lastpageonmobile", isMobileDevice && (page === pages.length - 1))
        ...
        <div className="custom-popup-wrapper">
            ...
            <div className="bottom-content">
                {!lastPageOnMobile ?
                    ...
                    : null}
                <div className={lastPageOnMobile ? "button-container fullwidth" : "button-container"}>
                    {page > 0 ? <button onClick={() => decreasePage()} className="popup-button prev"><FaChevronLeft />Previous
                    </button> : null}
                    {page < pages.length - 1 ? <button onClick={() => increasePage()} className="popup-button next">Next <FaChevronRight /> </button> : <button onClick={() => onClose()} className="popup-button final">Start exploring! <MdTravelExplore />
                    </button>}
                </div>
            </div>
        </div>
...
const ProgressIndicator = ({ currentPage, totalPages }: { currentPage: number, totalPages: number }) => {
    return (
        <div className="progress-indicator">
            {Array.from({ length: totalPages }, (_, index) => (
                <span
                    key={index}
                    className={`indicator-circle ${index === currentPage ? "active" : ""}`}
                />
            ))}
        </div>
    );
